...
#include "SKDActionSpaceDiscretizerMustMoveFront.hpp"
...
public:
    ...
    virtual bool load(const std::string& optionsFile) override {
        ...
        cout << "baseStoppingDist_: " << baseStoppingDist_ << endl;
        ...
        if(currentIntention == CAR_INTENTIONS::HAZARD_STOP){
            ...
            if(robotEnvironment_->isExecutionEnvironment()){
               // Sample an acceleration error
                ...
                brakingError = getUniformDistSample(-accelerationError * std::abs(brakingDeceleration_), accelerationError * std::abs(brakingDeceleration_));
            }
            ...
            FloatType speedError = getUniformDistSample(-velocityError * fixedVelocity_, velocityError * fixedVelocity_);
