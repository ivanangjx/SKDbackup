...
public:
    ...
    virtual int runEnvironment(int argc, char const * argv[]) {
        try {
            ...
            if (!oppt::fileExists(finalLogFilePath)) {
                ...
                // FloatType safeDistance = 0.0;
                // safeDistance = calculateInitialSafetyDistance();
                // cout << "Initial Radius: " <<  safeDistance << endl;
                ...
                // float runSuccessRate = 1.00;
                // float lowerVal = 0;
                // float higherVal = safeDistance;
                // float targetCollisionRate = float(1)- problemEnvironmentOptions_->safetyGoal;
                // float epsilon = 0.01f;      //0.01= 1%

                // while (true) {

                //     os << "New distance used: " <<  safeDistance << endl;
                ...
                //         os << "Run #" << i + 1 << endl;
                //         cout << "Run # " << i + 1 << endl;
                //         SimulationResult simulationResult = run(i + 1, os, argc, argv, safeDistance);
                //         totalDiscountedReward += simulationResult.discountedReward;
                //         totalNumSteps += simulationResult.stepsTaken;
                //         meanNumSteps += simulationResult.stepsTaken;
                //         meanPlanningTimePerStep += simulationResult.totalPlanningTime;
                ...
                //     os << "Mean number of steps: " << meanNumSteps << endl;
                //     os << "Mean planning time per step: " << meanPlanningTimePerStep * 1000 << "ms\n";
                //     os << "Num of runs: " << problemEnvironmentOptions_->nRuns << endl;
                //     os << "Num successful runs: " << numSuccessfulRuns << endl;
                    
                //     cout << "Num of runs: " << problemEnvironmentOptions_->nRuns << endl;                    
                //     cout << "Num successful runs: " << numSuccessfulRuns << endl;

                //     float runSuccessRate = float(numSuccessfulRuns)/(float(problemEnvironmentOptions_->nRuns));
                //     os << "Distance used: " <<  safeDistance << endl;
                //     os << "Collision rate: " << float(runSuccessRate) << endl;
                //     cout << "Collision rate: " << float(runSuccessRate) << endl;
                //     os << "Target Collision rate: <" << float(targetCollisionRate) << endl;
                //     cout << "Target Collision rate: <" << float(targetCollisionRate) << endl;
                //     os << "Average total discounted reward: " << totalDiscountedReward / (FloatType)problemEnvironmentOptions_->nRuns << endl;
                //     cout << "************************************************************************* \n";
                //     os << "************************************************************************* \n";

                //     os << "Fabs: " <<  fabs(runSuccessRate - targetCollisionRate) << " Epsilon: " <<  epsilon << endl;
                //     if ((fabs(runSuccessRate - targetCollisionRate) < epsilon) && (runSuccessRate < targetCollisionRate)) {
                //         break;
                //     }
                //     else if (runSuccessRate < targetCollisionRate) {
                //         lowerVal = safeDistance;
                //         safeDistance = (lowerVal + higherVal)/2;
                //     }
                //     else if (runSuccessRate > targetCollisionRate) {    //distance too big
                //         higherVal = safeDistance;
                //         safeDistance = (lowerVal + higherVal)/2;
                //     }
                // }
    ...
    // float calculateInitialSafetyDistance() {           //ivan: calculateInitialSafetyDistance
    //     // cout << "Calculating Initial Safety Distance..." << endl;

    //     FloatType safetyGoal = problemEnvironmentOptions_->safetyGoal;
    //     FloatType fixedStepTime = problemEnvironmentOptions_->fixedStepTime;
    //     FloatType brakingDeceleration = problemEnvironmentOptions_->brakingDeceleration;
    //     FloatType fixedVelocity = problemEnvironmentOptions_->fixedVelocity;

    //     // cout << "safetyGoal          : "<< safetyGoal << endl;
    //     // cout << "fixedStepTime       : "<< fixedStepTime << endl;
    //     // cout << "brakingDeceleration: "<< brakingDeceleration << endl;
    //     // cout << "fixedVelocity       : "<< fixedVelocity << endl;

    //     FloatType distance = (fixedVelocity * fixedStepTime) + 0.5*(brakingDeceleration)*(fixedStepTime)*(fixedStepTime);   //Distance travel from max velocity, given acceleration at the given timestep
    //     distance = distance * safetyGoal;
...
private:
    ...
    SimulationResult run(const unsigned int& run, std::ofstream & os, int argc, char const * argv[]) {
        ...
        RobotStateSharedPtr currentState =
            robotExecutionEnvironment_->sampleInitialState();       //sample and output an initial state
        ...
        auto viewer = robotExecutionEnvironment_->getRobot()->getViewer();      //output to viewer
        ...
        while (true) {
            ...
            cout << "action: " <<
                 *(robotPlanningEnvironment_->getRobot()->getActionSpace()->denormalizeAction(action))      //PRINTS BEST ACTION TO BE EXECUTED
                 << endl;
            ...
            if (terminal) {
                ...
                finalState = currentState;
                // cout << robotExecutionEnvironment_->getRewardPlugin()->getMinMaxReward().second << endl;
                if (reward == robotExecutionEnvironment_->getRewardPlugin()->getMinMaxReward().second) {        //initial checker-> doesnt account for same terminal/reward value - need to figure out
                // if (reward > 0) {
                    success = true;
                    cout << "success: " << success << endl;
                }   //ivan value for the mean time                   
                else {
            ...
            // Update the belief
            if (!terminal && !solver_->updateBelief(action, observationResult->observation, terminal)) {        //have to recheck this for particle filter afterwards
            ...
            //ivan- append expand info
            std::string distInfo = solver_->parseActionInfo();
            os << "constraint: " << distInfo << "\n";

            //ivan- append expand info
            std::string expandInfo = solver_->parseExpandInfo();
            cout << expandInfo << endl;
            os << expandInfo << "\n";
        ...
        cout << "Total time taken: " << totalTimeTaken * 1000.0 << "ms" << endl;
